...
2. At Step 3.5 (Level-Up Check):
   - Reads usage_log.md
   - Sees `last_amp_level_up_prompt: (not yet prompted)`
...
3. Adds to daily plan output:
   ...
   💡 **Tip:** You're using 1 of 25 Amp features. Run `/amp-level-up` to see what you might be missing.
   ```
4. Updates usage log metadata:
   ```markdown
   - **Last amp-level-up prompt:** 2026-01-28
   ```
...
grep "Last amp-level-up prompt" System/usage_log.md
# Expected: "- **Last amp-level-up prompt:** 2026-01-28"
```

---

## Test 3: User Runs /amp-level-up
...
**Action:** User types `/amp-level-up`
...
2. Amp guides them through `/review` command
...
**Next Time User Runs `/amp-level-up`:**
...
# Next /amp-level-up won't suggest daily review again
# (already checked)
```
...
2. At Step 3.5 (Level-Up Check):
   - Reads `last_amp_level_up_prompt: 2026-01-28`
...
You're using Amp like a pro! 🎉 Here are **2 advanced capabilities** that could 
...
**How to start:** Run `/amp-improve` and describe what you wish worked differently.
...
- Test `/amp-level-up` still recommends correctly

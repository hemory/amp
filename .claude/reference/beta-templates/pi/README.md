...
```
Build me a tool that lists all my overdue tasks from the Amp task system
```
...
## How Pi Connects to Amp

Pi has **native MCP (Model Context Protocol) support** — it can directly access your Amp MCP servers without any bridge code.
...
For **deterministic operations** (task CRUD, calendar writes), continue using Amp directly — reliability matters more than flexibility there.
...
Pi connects directly to these Amp MCP servers:
...
- And all other Amp MCP servers

These are configured automatically during beta activation.

### AGENTS.md

Pi's context about your Amp vault is in `.pi/AGENTS.md`. Edit this to give Pi more context about your specific setup.
...
If servers aren't connecting, verify your `.pi/config.json` has the Amp MCP servers configured.
...
- MCP bridge to Amp task and calendar systems

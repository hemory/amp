...
export async function extractCommitments(): Promise<void> {
  ...
  const meetingNotes = await amp.getMeetingNotes({ since: cutoffDate });

  for (const meeting of meetingNotes) {
    const content = await amp.readFile(meeting.path);
  ...
  const slackFiles = await amp.glob('00-Inbox/Slack/**/*.md');
  for (const slackFile of slackFiles) {
    const content = await amp.readFile(slackFile);
...
Your best ideas might become built-in Amp features.

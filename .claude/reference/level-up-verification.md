...
**Purpose:** Verify that usage tracking works correctly to power `/amp-level-up` recommendations.
...
```
User Action → Command Execution → Usage Log Update → /amp-level-up Analysis → Recommendations
```
...
CLAUDE.md (lines 173-186) instructs Amp to track usage automatically:

> **Usage Tracking (Silent)**
> Track feature adoption in `System/usage_log.md` to power `/amp-level-up` recommendations:
> 
...
**Verification:**
- `/amp-level-up` can read file system to detect usage
...
## Tracking Metadata
- Last amp-level-up prompt: (not yet prompted)
- First daily plan: 2026-01-21
- Setup date: 2026-01-20
```

**Expected `/amp-level-up` Output:**
...
2. `/amp-level-up` correctly counts 2 of ~25 features used
...
- [ ] System improvements (/amp-improve)
- [ ] Prompt improvement (/prompt-improver)
- [x] Demo mode (/amp-demo)
```
...
2. **System Customization** - Make Amp yours

Celebration message: "You're using Amp like a pro! 🎉"
...
### ✅ 2. /amp-level-up Can Read Usage Log

**Test:**
- `/amp-level-up` command reads `System/usage_log.md`
- Parses markdown checkboxes correctly
- Counts checked vs unchecked items

**Files:**
- `.claude/skills/amp-level-up/SKILL.md` Step 1 (read logic)
...
**Files:**
- `.claude/skills/amp-level-up/SKILL.md` Step 2 (pattern analysis)
- `.claude/skills/amp-level-up/SKILL.md` Step 3 (recommendation generation)
...
- After prompt: `last_amp_level_up_prompt` date updated
...
**Expected:**
- `/amp-level-up` shows: "Looks like you're just getting started!"
- Recommends 3 essentials: daily planning, task management, meeting capture

### All Features Used

**Expected:**
- `/amp-level-up` shows celebration: "You're using every feature in Amp! 🎉"
- Suggests custom MCPs, system improvements, sharing knowledge

### User Says "Show Me Everything"

**Expected:**
- `/amp-level-up` says: "I could, but that's overwhelming..."
...
- [x] Created `/amp-level-up` command with pattern analysis logic
- [x] Added README section highlighting feature discovery
- [x] Added onboarding mention of `/amp-level-up`
- [x] Added smart trigger to `/daily-plan` (7-day cooldown)
- [x] Tested scenario recommendations (examples in amp-level-up.md)
...
4. Update `/amp-level-up` command if new progression patterns emerge

...
Amp includes eight custom MCP servers in `core/mcp/`:
...
4. Amp summarizes: "Last week you discussed Q1 roadmap priorities. Sarah mentioned hiring concerns for the design team. Follow up on design headcount."
...
### Amp Improvements MCP (`improvements_server.py`)

**What it does:**  
Capture and track Amp system improvement ideas with automatic duplicate detection. Powers the `/amp-backlog` workflow.

**Why it's an MCP:**  
You want to capture improvement ideas from any context (during reviews, while planning, mid-conversation) without context switching. The Amp Improvements MCP provides instant capture with automatic ID generation and similarity checking.
...
You mention this → Amp Improvements MCP:
...
5. Saves to `System/Amp_Backlog.md` with category: `tasks`
6. Next time you run `/amp-backlog`, AI ranks it against other ideas
...
- **Validation enforcement** - Cannot skip required fields (especially Step 3: email domain)
...
- **Deferred onboarding extras** - Leaves calendar analysis and richer setup for `/getting-started`

**Real-world example:**  
New user runs onboarding → provides name and role → **tries to skip email domain** → Onboarding MCP blocks progression: "Email domain is required for Internal/External person routing." → User provides domain and communication preferences → finalization creates the vault structure, user profile, and a temporary General pillar bucket so they can start working immediately.

**Tools:** `start_onboarding_session`, `validate_and_save_step`, `get_onboarding_status`, `verify_dependencies`, `finalize_onboarding`, `complete_obsidian_walkthrough`, `check_onboarding_complete`
...
**What it does:**  
GitHub update detection for `/amp-update` and `/amp-rollback`. Checks Amp repository for new releases, parses changelogs, and manages version comparison.
...
**Real-world example:**  
User runs `/amp-update` → Update Checker MCP checks GitHub → finds v2.1.0 with new features → shows changelog with "Added Obsidian integration, improved onboarding" → user confirms → creates backup → pulls updates → installs dependencies → success message with "Run `/getting-started` to explore new features."
...
| Amp Improvements | `improvements_server.py` | Built-in |
...
See `.mcp.json.template` for a complete config with all built-in servers:
...
- `improvements_server.py` - System improvement backlog
...
```json
{
  "mcpServers": {
    "work-mcp": {
      "command": "python",
      "args": ["/path/to/amp/core/mcp/work_server.py"],
      "env": { "VAULT_PATH": "/path/to/amp" }
    },
    "amp-improvements": {
      "command": "python",
      "args": ["/path/to/amp/core/mcp/improvements_server.py"],
      "env": { "VAULT_PATH": "/path/to/amp" }
    }
  }
}
...
When you run `/amp-update`, Amp preserves any MCP entries named `user-*` or `custom-*`. Your custom integrations will never be overwritten by updates.

If you name an MCP server without this prefix (e.g., `gmail-mcp`) and a future Amp update adds a server with the same name, you'll be asked which version to keep. Using the prefix avoids this conflict entirely.

---

## Background Automation

Amp includes background automation that runs independently of Claude/Cursor, enabling the system to learn continuously.
...
5. Session start hook displays prompt to run `/amp-whats-new`
...
3. If 5+ pending learnings:
   ...
   - Session start hook displays count and suggests `/amp-whats-new --learnings`
...
- `com.amp.changelog-checker.plist` - Runs every 6 hours
- `com.amp.learning-review.plist` - Runs daily at 5pm

**Verify installation:**
```bash
launchctl list | grep com.amp
```

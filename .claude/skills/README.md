...
Amp includes two categories of skills:

### Amp Skills (PKM-Specific)

Built specifically for personal knowledge management and productivity workflows in Amp:
...
- `/amp-level-up` - Discover unused features
- `/amp-backlog` - AI-powered idea ranking
- `/amp-improve` - Workshop improvement ideas
- `/amp-whats-new` - Check for system improvements (learnings + Claude updates)
- `/amp-show-changes` - Preview production changes before updating, without applying them
- `/amp-update` - Update Amp automatically (shows what's new, updates if confirmed, no technical knowledge needed)
- `/amp-rollback` - Undo last update if something went wrong
- `/amp-obsidian-setup` - Enable Obsidian integration and migrate vault to wiki links
...
When writing skill instructions, use consistent placeholder syntax:

| Syntax | Meaning | Example |
|--------|---------|---------|
| `{{var}}` | Replaced by install/setup tooling | `{{VAULT_PATH}}/core/mcp/work_server.py` |
| `$VAR` or `$CLAUDE_PROJECT_DIR` | Environment variable resolved at runtime | `bash "$CLAUDE_PROJECT_DIR/System/scripts/test.sh"` |
| `YYYY-MM-DD` | Dynamic value the agent fills in | `## Update - YYYY-MM-DD` |
| Descriptive text | Contextual instruction the agent interprets | `<github-username-from-git-config>` |

Do **not** use `[PLACEHOLDER]` or `YOUR_USERNAME` in shipped skills. These look like unresolved template variables and confuse both the agent and users.

---

## Feature Flags (Beta System)

Beta features are defined in `.claude/config/beta-features.yaml` and activated via `/beta-activate`. To check a feature flag in a skill:

```
1. Read System/user-profile.yaml
2. Check if beta.<feature_key>.activated is true
3. If active, enable the feature's capabilities
4. If not, skip or suggest the user run /beta-activate
```

Available flags are listed in `beta-features.yaml` under each feature's `feature_flags:` section.

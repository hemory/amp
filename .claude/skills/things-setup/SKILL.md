---
name: things-setup
description: Connect Things 3 to Amp for bi-directional task sync
manifest:
  id: things
  auth: none
  category: task_sync
...
Connect Things 3 to Amp for bi-directional task sync. No account needed. Everything stays on your Mac. Works offline.

## What This Enables

Once connected, Amp can:
- **Task Sync:** Create tasks in Things 3 when you add them in Amp, and vice versa
...
- **Pillar Mapping:** Amp pillars map to Things Areas for organized task views
...
```
I need to add the Things 3 connector to your Amp configuration.
...
Map Amp pillars to Things Areas:

```
Now let's map your Amp pillars to Things Areas.
...
1. **Auto-sync** — Tasks sync automatically between Amp and Things
...
- **Task creation** in Amp mirrors to Things (P0/P1 → Today, P2/P3 → Anytime)
- **Task completion** syncs both ways — finish in either app
- **Pillars → Areas** keep your Things organized by Amp structure
...
If a task is edited in both Amp and Things between syncs:

- Amp is the source of truth for task status
- Things is the source of truth for task title edits
- Notes merge (Amp context is appended, not overwritten)

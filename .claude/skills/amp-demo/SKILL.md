...
- `/amp-demo off` - Disable demo mode
...
Enter a number (1-12) or type `/amp-demo menu` to see this again.
```

### For `/amp-demo menu`

1. Check if demo mode is enabled
2. If not, error: "Demo mode is not enabled. Run `/amp-demo on` first."
...
- **Scenarios 1-4, 7-8, 11-12:** Run the actual commands (`/journal`, `/daily-plan`, `/review`, `/triage`, `/week-plan`, `/week-review`, `/career-coach`, `/amp-backlog`, `/amp-whats-new`)

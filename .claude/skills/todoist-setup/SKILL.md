---
name: todoist-setup
description: Connect Todoist to Amp for two-way task sync
integration:
  id: todoist
  name: Todoist
  ...
  enhances:
    - skill: daily-plan
      capability: "Merges Todoist tasks due today alongside Amp tasks"
    - skill: triage
      capability: "Routes items to Amp, Todoist, or both"
    - skill: process-inbox
      capability: "Creates tasks in both systems when processing captured items"
    - skill: week-review
      capability: "Shows cross-system completion stats (Amp + Todoist)"
  new_capabilities:
...
Connect your Todoist account to Amp so tasks stay in sync across both systems. Create a task in Amp and it appears in Todoist. Complete one in Todoist and Amp knows about it.

## What This Enables

Once connected, Amp can:
- **Daily Plan** (`/daily-plan`): Merge Todoist tasks due today alongside your Amp tasks
- **Triage** (`/triage`): Route new items to Amp, Todoist, or both
- **Process Inbox** (`/process-inbox`): Same routing options when processing captured items
- **Week Review** (`/week-review`): See completion stats across both Amp and Todoist

## How Sync Works (Plain English)

- When you create a task in Amp, it gets pushed to Todoist automatically
- When you complete a task in Todoist, Amp picks it up during your next daily plan
...
- Tasks created by Amp carry a `[amp:task-ID]` marker in the description so the systems never duplicate

## Privacy

- Task titles and status sync between Amp and Todoist. No task content or notes are stored beyond what you already have in both systems.
- Your API key stays local on your machine in `System/integrations/config.yaml` (gitignored)
- Amp never shares your Todoist data with third parties
- Tasks created in Todoist are only pulled into Amp if they were NOT originally pushed from Amp (loop prevention via `[amp:...]` marker)
...
```
**Let's connect Todoist to Amp.**

Two-way task sync. Create in Amp — appears in Todoist. Complete in Todoist — done in Amp.
...
> **Security note:** Your API token will be stored locally in `System/integrations/config.yaml` (gitignored). Never paste tokens directly in terminal commands as they persist in shell history.
...
This lets Amp talk to Todoist using your API token.
```

2. Add to the user's `.mcp.json` (use the `/amp-add-mcp` skill or manual edit):
...
Ask the user which Todoist project should receive Amp tasks:

```
**Which Todoist project should Amp tasks go into?**
...
You can pick one default project, or map each Amp pillar to a different project.

**Option A:** All Amp tasks go to one project (simplest)
...
- **`/daily-plan`** → Merges Todoist tasks due today alongside your Amp tasks.
  Externally-created tasks show a [Todoist] label so you know where they came from.

- **`/triage`** → Now offers routing to Amp, Todoist, or both when processing items.

- **`/process-inbox`** → Same routing — "This looks like a task. Add to Amp, Todoist, or both?"
...
These work automatically starting now. Run `/amp-level-up` anytime to see what else you can do.
```
...
- **Check the project mapping** — if your Amp pillar maps to a project that was renamed or deleted in Todoist, tasks may go to the Inbox instead.
...
- **Amp-originated tasks** should have `[amp:task-...]` in their Todoist description. The adapter skips these during pull-in to prevent loops.
- **Todoist-originated tasks** that you manually add to Amp won't have a mapping in `.sync-state.json` and may get pulled again. Mark them in Amp to create the mapping.
...
1. Update `System/integrations/config.yaml`:
   ...
   todoist:
     enabled: false
   ```
2. Confirm: "Todoist is disconnected. Tasks will no longer sync between systems. Your existing tasks in both Amp and Todoist are unchanged. Run `/todoist-setup` anytime to reconnect."

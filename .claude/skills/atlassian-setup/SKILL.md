...
description: Connect Jira and Confluence to Amp for project tracking and knowledge search
manifest:
  id: atlassian
  auth: oauth2
...
Connect your Jira and Confluence to Amp so your daily plans, project health checks, meeting prep, and weekly reviews get richer context from your Atlassian workspace.
...
- **Task Sync:** Bi-directional sync between Amp tasks and Jira issues (optional)

## Privacy

- Amp reads Jira issues and Confluence pages **on demand** -- nothing is stored permanently
...
```
**Let's connect Jira and Confluence to Amp.**
...
```
I need to add the Atlassian connector to your Amp configuration.
...
Which site should Amp connect to?
```
...
```
**Which Jira project should Amp sync with?**
...
If connected, Amp will search Confluence docs when prepping for meetings --
...
```
**How should Amp handle Jira task sync?**

When you create tasks in Amp or complete issues in Jira, Amp can keep them in sync.

Choose your trust level:

1. **Auto-sync** -- Amp automatically syncs issues and tasks (recommended for personal projects)
2. **Ask each time** -- Amp shows you what changed and asks before syncing
3. **Read-only** -- Amp reads Jira data but never writes back (safest)

...
1. **Missing packages:** Run `pip install -e amp-core` from the amp-core directory
2. **Missing .mcp.json:** Copy from `.mcp.json.template`, substitute VAULT_PATH
...
- **Stale inbox files** (>30 days in `00-Inbox/`)
- **Broken WikiLinks** (dead `[[links]]` in PARA folders)
- **Orphaned person pages** (people not referenced in tasks or recent meetings)
- **Stale agent memory** (>90 day old files in `.claude/memory/`)

Include results in the health report under a **Vault Hygiene** section:
...
### Step 7: Fresh Re-Check (Optional)
...
2. Run the pre-flight checker:
   ```bash
   python3 "$VAULT_PATH/amp-core/core/utils/preflight.py" 2>/dev/null || python3 "core/utils/preflight.py" 2>/dev/null
   ```
...
- `/amp-update` — Update Amp (often fixes package issues)
- `/xray` — Understand how Amp's architecture works under the hood

...
## Step 0: Drop Zone Check

Before starting triage, check if today's drop zone has unprocessed items:

1. Look for today's drop zone file. Check the new format first, then fall back to legacy:
   - **New format:** `00-Inbox/Drop_Zone/YYYY-MM-DD - Drop Zone.md`
   - **Legacy format:** `00-Inbox/Drop_Zone/YYYY-MM-DD-Drop-Zone.md`
   Use whichever exists (prefer new format if both exist).
2. If the file exists, read it and count entries that do NOT have `✅ Processed`
3. **If unprocessed items exist:**
   > "📋 You have [X] unprocessed items in your drop zone. Want me to process them first?"
   - If user says yes: Run the `/capture` workflow, then continue with triage
   - If user says no or skips: Continue with triage
4. **If no unprocessed items or file doesn't exist:** Continue silently

---

## Step 1: Load Strategic Context & Structure Discovery

---
name: trello-setup
description: Connect Trello to Amp for visual Kanban task sync
manifest:
  id: trello
  auth: api_key_token
  category: task_sync
...
Connect your Trello boards to Amp so your tasks, projects, and daily plans stay in sync with your Trello Kanban boards.

## What This Enables

Once connected, Amp can:
- **Task Sync:** Cards created in Amp appear on your Trello board; cards moved to Done in Trello mark tasks complete in Amp
...
```
**Let's connect Trello to Amp.**

This links your Trello boards so tasks sync between Amp and Trello.
Cards move between lists = status updates in Amp. Simple Kanban sync.
...
> **Security note:** Credentials are stored locally in `System/integrations/config.yaml` (gitignored). Ensure `.mcp.json` is also in your `.gitignore`. Never paste tokens directly in terminal commands as they persist in shell history.
...
```
I'll add the Trello connector to your Amp configuration.
...
```
**Which Trello board should Amp sync with?**
...
```
**Now let's map your lists to Amp statuses.**
...
```
**How should Amp handle Trello sync?**

1. **Auto-sync** — Cards sync automatically during daily plan and task creation
2. **Ask each time** — Amp shows you what changed and asks before syncing
...
- **Task Sync** — New Amp tasks appear on your [Board Name] board. Cards completed in Trello mark tasks done in Amp.

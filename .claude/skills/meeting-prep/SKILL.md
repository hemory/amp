...
#### Context Enrichment Note

After completing attendee lookup, if any attendees are missing person pages or have sparse pages (no role, no recent interactions, no notes), add this hint to the prep output:

> 💡 I don't have much context on [attendee name(s)]. After the meeting, run `/process-meetings` to build their profile, or create a person page at `05-Areas/People/`.

Only show this for attendees WITHOUT existing person pages or with very sparse pages. Don't show for attendees who already have rich person pages. This hint is informational and does not block meeting prep.

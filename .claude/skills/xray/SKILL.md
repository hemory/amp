...
**With qualifiers:** Deep dive into specific topics (AI fundamentals, Amp architecture, how to extend).
...
3. **Connect to concepts** - Link specific actions to how AI/Amp works
...
| `/xray amp` or `/xray architecture` | How Amp works: CLAUDE.md, hooks, MCPs, skills |
| `/xray boot` or `/xray session start` | The session startup sequence |
| `/xray today` | ScreenPipe analysis of your day |
| `/xray extend` or `/xray customize` | How to customize and build on Amp yourself |
...
- `/xray amp` — Full architecture: CLAUDE.md, hooks, MCPs, skills, vault
...
Let me explain the core concepts that underpin everything Amp does.
...
**So how does Amp remember things?** We engineer around the limitation. 
That's what Amp is — clever engineering to give a forgetful AI the *appearance* 
...
**Amp's job:** Get the RIGHT information INTO that window at the RIGHT time.
...
**Amp's approach:** Selective loading. Don't dump everything — load what's 
...
the **system prompt**. For Amp, that's `CLAUDE.md`.

```
┌─────────────────────────────────────────────────┐
│ CLAUDE.md (loaded before you type anything)     │
├─────────────────────────────────────────────────┤
│ "You are Amp, a personal knowledge assistant"   │
...
In Amp, tools come from **MCP servers** (Model Context Protocol). Each MCP 
...
1. **Know why Amp loads things at session start** — to get context into the window
...
**Want to see Amp's specific architecture?** Try `/xray amp`
```

---

## MODE: Amp Architecture (`/xray amp`)

**Purpose:** Explain the specific building blocks that make Amp work.

### Output Format

```markdown
## 🏗️ How Amp Works: The Architecture

Now that you understand AI fundamentals, here's how Amp engineers around 
...
The system prompt that defines who Amp is and how it behaves.
...
- Activity patterns
- "Moments you missed" where Amp could have helped
```
...
**This survives updates.** When you run `/amp-update`, this block is preserved.
...
**Amp is scaffolding, not a finished product.** It's a starting point you customize.

The goal isn't to use Amp "as is" forever — it's to understand it well 
...
- `/xray amp` → Check "Understands system prompts", "Understands tools/MCPs"

...
Enable ambient work intelligence - Amp learns what you're working on via screen OCR.
...
2. If NOT activated:
   ...
   /beta-activate AMPSCREENPIPE2026
...
```bash
# Copy cleanup script from amp-core
cp amp-core/core/scripts/screenpipe-cleanup.sh ~/.screenpipe/cleanup.sh
chmod +x ~/.screenpipe/cleanup.sh

# Install daily cleanup (runs at 3 AM)
cp amp-core/core/scripts/com.amp.screenpipe-cleanup.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.amp.screenpipe-cleanup.plist
```
...
- Yes: `mv ~/.screenpipe/ ~/.Trash/screenpipe-backup-$(date +%Y%m%d)/ 2>/dev/null || true`

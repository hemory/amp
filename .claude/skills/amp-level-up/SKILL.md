---
name: amp-level-up
description: Discover unused Amp features based on your usage patterns
---

## Purpose

Discover Amp capabilities you haven't used yet. No FOMO — this is your concierge showing you what's available so you can get full value from the system without missing features.
...
Check if the user's vault has mentions of tools they haven't connected:

- Search vault files for references to Slack, Todoist, Google Calendar, Zoom, etc.
- If significant mentions found that aren't already connected:
...
If no high_value items found, skip this section entirely. Don't show moderate or available items here — keep `/amp-level-up` focused on high-signal recommendations only.
...
```markdown
# 🚀 Level Up Your Amp System

Based on your usage, here are **{{X}} ways to get more value** from Amp:
...
*Run `/amp-level-up` anytime to see what else you might be missing.*
```
...
Based on your usage, here are **3 ways to get more value** from Amp:
...
You're using Amp like a pro. Here are **2 advanced features** that could push your system even further:
...
**Why it's relevant:** You've built 23 person pages and you're consistent with relationship tracking. Automating the data sync would save you 15-20 minutes per week and ensure your Amp system stays in sync with your CRM.
...
> "You're using every feature in Amp! 🎉
> 
> At this point, consider:
> - Building custom MCPs for your specific workflow
> - Running `/amp-improve` to suggest system enhancements
...
> - `06-Resources/Amp_System/Amp_System_Guide.md` for the complete guide"
...
> ---
> 
> **💡 Have your own ideas for improving Amp?**
> 
...
> 
> Run `/amp-backlog` to see your ideas ranked, or `/amp-improve` to workshop one.
> 
...
If user hasn't run `/amp-level-up` in 7+ days and has unused features, add a gentle nudge:

```markdown
---

💡 **Tip:** You're using {{X}} of {{Y}} Amp features. Run `/amp-level-up` to see what you might be missing.
```
...
> "One more thing: run `/amp-level-up` anytime to discover features you haven't tried yet. No FOMO — just helpful guidance on what's available."

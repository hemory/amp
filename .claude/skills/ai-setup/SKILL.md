---
name: ai-setup
description: Configure AI model options - budget cloud models (save 80%+) and offline mode (use Amp without internet)
---

## Purpose

Help users configure alternative AI models for Amp:
...
Right now, Amp uses Claude for everything. It's excellent, but it costs money per use and requires internet.
...
**1. 💰 Budget Cloud Mode** — Cheaper AI when online
   ...
   **Best for:** People who use Amp heavily and want to cut costs
...
What happens now:
- When you have internet → Amp uses Claude (best quality)
- When you're offline → Amp can use your local model
...
- Run `bash "$CLAUDE_PROJECT_DIR/System/scripts/test-ai-connections.sh"` silently
...
- When you're online: Amp uses Claude (best quality)

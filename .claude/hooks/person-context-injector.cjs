...
// Copilot CLI compat: toolArgs is a JSON string, tool_input is an object
let _parsedArgs = input.tool_input || {};
try { if (typeof input.toolArgs === 'string') _parsedArgs = JSON.parse(input.toolArgs); } catch(e) {}
const filePath = _parsedArgs.path || _parsedArgs.file_path || input.tool_input?.path || input.tool_input?.file_path || '';
...
function parsePersonPage(filePath) {
  try {
    ...
    // Fallback: parse bold markdown fields (e.g., **Role:** Senior PM)
    if (!info.role) {
      const mdRole = content.match(/\*\*Role:\*\*\s*(.+)/);
      if (mdRole) info.role = mdRole[1].trim();
    }
    if (!info.company) {
      const mdCompany = content.match(/\*\*Company:\*\*\s*(.+)/);
      if (mdCompany) info.company = mdCompany[1].trim();
    }
    if (!info.name || info.name === fileName.replace(/_/g, ' ')) {
      const h1 = content.match(/^#\s+(.+)$/m);
      if (h1) info.name = h1[1].trim();
    }

...
**Status:** Claude Code hooks live here. GitHub Copilot CLI support lives in `.github/hooks/` as an adapter layer that can call these scripts.
...
| Environment | Hook config path | Implementation path | Notes |
|-------------|------------------|---------------------|-------|
| **Claude Code** | `.claude/settings.json` or user Claude settings | `.claude/hooks/` | Uses Claude Code hook event names, stdin payloads, and exit codes. |
| **GitHub Copilot CLI** | `.github/hooks/hooks.json` | `.github/hooks/` adapters | Adapters normalize Copilot CLI payloads, set `CLAUDE_PROJECT_DIR` when needed, and call compatible `.claude/hooks/` scripts. |
| **Other runtimes** | Runtime-specific | Runtime-specific | Use explicit skills or commands unless that runtime documents hook support. |

## Copilot CLI adapter notes

Copilot CLI and Claude Code use different hook payload field names. The adapters in `.github/hooks/` accept Copilot CLI shapes such as `toolName`/`toolArgs`, `tool_name`/`tool_args`, and `tool`/`args`, then translate them to the Claude Code-style `tool_name` and `tool_input` payload expected by these scripts.

Keep both hook systems side by side unless a migration is explicit and tested. `.claude/hooks/` remains the Claude Code implementation; `.github/hooks/` is the Copilot CLI compatibility layer. See `.github/hooks/README.md` and `docs/copilot-cli.md` for details.

The `copilot-*` files under `.claude/hooks/` are retained as compatibility copies for older checkouts and should stay byte-for-byte aligned with the `.github/hooks/` versions until they are formally removed.
...
4. Desktop notification: "✅ Amp finished - 20 meetings processed"
    ...
    "Notification": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Amp needs your input\" with title \"Amp\"'"
          }
        ]
      }
    ]
...
- **Trigger:** Manual (`node .claude/hooks/maintenance.cjs`) or ask Amp to "run vault maintenance"

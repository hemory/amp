...
// CLAUDE_HOOK_CONTEXT is set by Claude Code runtime but not by Copilot CLI.
// This hook is currently non-functional in Copilot CLI environments.
const hookContext = process.env.CLAUDE_HOOK_CONTEXT;
if (!hookContext) {
  process.exit(0);
}

const input = JSON.parse(hookContext);

...
function checkOrphanedPages() {
  ...
  const walkPeople = (dir) => {
    ...
    for (const entry of entries) {
      ...
      else if (entry.name.endsWith('.md') && entry.name !== 'README.md') personFiles.push(fullPath);
    }
  };
  ...
  for (const pf of personFiles) {
    ...
    if (fs.existsSync(tasksPath)) {
      try {
        ...
        if (tasksContent.includes(name) || tasksContent.includes(spacedName)) referenced = true;
      } catch (e) { /* read error */ }
    }

    // Check meeting notes (sample recent 20)
    if (!referenced) {
      const meetingDir = path.join(vaultRoot, '00-Inbox/Meetings');
      if (fs.existsSync(meetingDir)) {
        try {
          ...
          for (const m of meetings) {
            ...
            if (mc.includes(name) || mc.includes(spacedName)) { referenced = true; break; }
          }
        } catch (e) { /* read error */ }
      }
    }

    // Check project files (sample recent 20)
    if (!referenced) {
      const projectDir = path.join(vaultRoot, '04-Projects');
      if (fs.existsSync(projectDir)) {
        try {
          const projFiles = [];
          const walkProjects = (dir, depth) => {
            if (depth > 3) return;
            const entries = fs.readdirSync(dir, { withFileTypes: true });
            for (const e of entries) {
              if (e.name.startsWith('.')) continue;
              const fp = path.join(dir, e.name);
              if (e.isDirectory()) walkProjects(fp, depth + 1);
              else if (e.name.endsWith('.md')) projFiles.push(fp);
            }
          };
          walkProjects(projectDir, 0);
          for (const pFile of projFiles.slice(0, 20)) {
            const pc = fs.readFileSync(pFile, 'utf-8');
            if (pc.includes(name) || pc.includes(spacedName)) { referenced = true; break; }
          }
        } catch (e) { /* read error */ }
      }
    }
...
// === Run All Checks ===
console.log('Amp Vault Maintenance\n');

 ...
 * 
 * Detection methods:
 * - File paths containing 05-Areas/Companies/
 * - Company names matching files in companies folder
 * 
...
if (!filePath || filePath.includes('/05-Areas/Companies/')) {
  process.exit(0);
}
...
try {
  ...
  // Method 1: File path references (05-Areas/Companies/)
  const fileRefPattern = /05-Areas\/Companies\/[^\s]*?([A-Za-z0-9_-]+)(?:\.md)?/g;
  let match;

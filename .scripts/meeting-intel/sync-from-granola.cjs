...
const TOKEN_FILE = path.join(os.homedir(), '.config', 'amp', 'granola-tokens.json');
...
---
*Processed by Amp Meeting Intel (${meeting.source === 'mcp' ? 'MCP' : 'Cache'} source)*
`;
...
async function main() {
  ...
  log('='.repeat(60));
  log('Amp Meeting Intel - Granola Sync (MCP-first)');
  log('='.repeat(60));

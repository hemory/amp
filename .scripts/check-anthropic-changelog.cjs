 ...
 * Writes alert file when updates detected, prompting user to run /amp-whats-new.
 * 
...
async function detectChanges(state) {
  ...
  // This is intentionally simple - full analysis happens in /amp-whats-new
...
Run \`/amp-whats-new\` to:
- See what's new since your last check
- Get specific suggestions for how to use new features in Amp
...
async function main() {
  ...
  const newState = {
    ...
    features_seen: state.features_seen // Preserved, updated by /amp-whats-new
  };

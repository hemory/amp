...
def log_event_locally(event_name: str, properties: dict, visitor_id: str):
    ...
    try:
        ...
        logger.debug("Failed to write analytics log: %s", e)


# Create MCP server
server = Server("amp-analytics")
...
async def list_tools() -> list:
    ...
    return [
        Tool(
            name="track_event",
            description="Track a Amp usage event. Only fires if user has opted into analytics.",
            inputSchema={
                "type": "object",
                "properties": {
                    "event_name": {
...
async def _call_tool_inner(name: str, arguments: dict) -> list[TextContent]:
    if name == "check_analytics_status":
        ...
        if not HAS_REQUESTS:
            return [TextContent(type="text", text=json.dumps(
                mcp_error("requests library not installed. Run: pip install requests")))]

        transport = get_analytics_transport()
        if not transport.get("configured"):
            return [TextContent(type="text", text=json.dumps(
                mcp_error(f"Analytics transport not configured ({transport.get('reason', 'unknown')}).")))]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return [TextContent(type="text", text=json.dumps({
                    **mcp_error(f"Test event failed with status {response.status_code}"),
                    "status": response.status_code,
                    "transport_mode": transport.get("mode"),
                    "transport_endpoint": transport.get("endpoint"),
            ...
            logger.warning("Test connection request failed: %s", e)
            return [TextContent(type="text", text=json.dumps(
                mcp_error(str(e))))]

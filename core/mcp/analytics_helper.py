#!/usr/bin/env python3
"""
Amp Analytics Helper

Shared utilities for analytics across Amp skills and MCPs.
Handles consent checking, journey metadata calculation, and event firing.

Privacy Principles:
- Only tracks Amp built-in features, not user customizations
...
def get_analytics_mode() -> str:
    ...
    - proxy:  client sends to a relay endpoint (requires AMP_ANALYTICS_ENDPOINT)
    """
    mode = os.environ.get('AMP_ANALYTICS_MODE', ANALYTICS_MODE_DIRECT).strip().lower()
...
def get_analytics_endpoint(mode: Optional[str] = None) -> str:
    ...
    endpoint = os.environ.get('AMP_ANALYTICS_ENDPOINT', '').strip()
...
def get_proxy_token() -> Optional[str]:
    ...
    token = os.environ.get('AMP_ANALYTICS_PROXY_TOKEN', '').strip()
...
def get_analytics_transport() -> Dict[str, Any]:
    ...
    if mode == ANALYTICS_MODE_DIRECT:
        ...
        headers['x-amp-analytics-client'] = 'amp-core'
...
def calculate_journey_metadata() -> Dict[str, Any]:
    ...
    areas = {
        ...
        'discovery': [
            ...
            'Improvement workshop', 'Idea captured', 'Amp updated', 'Learnings reviewed'
        ],
        'integrations': [
    ...
    if setup_date_str and setup_date_str not in ['(set during onboarding)', '']:
        try:
            ...
            logger.debug("Failed to parse setup_date: %s", setup_date_str)
            days = 0
    else:
        days = 0
...
def get_visitor_info() -> Dict[str, str]:
    ...
    account_id = analytics.get('account_id') or profile.get('email_domain', 'amp-users')

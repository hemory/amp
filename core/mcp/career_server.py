#!/usr/bin/env python3
"""
Career Development MCP Server for Amp
...
from core.utils import validate_input, mcp_error, validate_vault_path

# Import parsing utilities
from core.mcp.career_parser import (
    parse_evidence_file,
    parse_ladder_file,
...
async def handle_scan_evidence(arguments: dict) -> list[types.TextContent]:
    ...
    if not EVIDENCE_DIR.exists():
        return [types.TextContent(
            type="text",
            text=json.dumps({
                **mcp_error(f"Evidence directory not found: {EVIDENCE_DIR}"),
...
async def handle_parse_ladder(arguments: dict) -> list[types.TextContent]:
    ...
    if not LADDER_FILE.exists():
        return [types.TextContent(
            type="text",
            text=json.dumps({
                **mcp_error(f"Career ladder file not found: {LADDER_FILE}"),
    ...
    if "error" in ladder_data:
        return [types.TextContent(
            type="text",
            text=json.dumps({
                **mcp_error(ladder_data.get("error", "Unknown error")),
                **ladder_data
            }, indent=2)
        )]
...
async def handle_analyze_coverage(arguments: dict) -> list[types.TextContent]:
    ...
    if not EVIDENCE_DIR.exists():
        return [types.TextContent(
            type="text",
            text=json.dumps({
                **mcp_error("Evidence directory not found"),
                "note": "Run /career-setup to initialize your career system"
            }, indent=2)
        )]
    ...
    if not LADDER_FILE.exists():
        return [types.TextContent(
            type="text",
            text=json.dumps({
                **mcp_error("Career ladder file not found"),
                "note": "Run /career-setup to create your career ladder"
            }, indent=2)
        )]
    ...
    if "error" in ladder_data or not ladder_data.get('competencies'):
        return [types.TextContent(
            type="text",
            text=json.dumps({
                **mcp_error("Failed to parse career ladder or no competencies found"),
            ...
            for period_info in period_data:
                ...
                for evidence in period_info['files']:
                    ...
                    for comp in ladder_data['competencies']:
                        ...
                        from core.mcp.career_parser import match_evidence_to_competency
...
async def handle_generate_evidence_from_work(arguments: dict) -> list[types.TextContent]:
    ...
    try:
        work_id = validate_input(arguments['work_id'], 'work_id', max_length=100)
        work_type = validate_input(arguments['work_type'], 'work_type', max_length=50)
...
"""
    ...
    # 3. Write file (validate path to prevent traversal via title)
    try:
...
# MAIN
    ...
    logger.info("Starting Amp Career MCP Server")

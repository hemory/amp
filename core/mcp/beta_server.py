#!/usr/bin/env python3
"""
MCP Server for Amp Beta Features System
...
def setup_pi_integration() -> bool:
    ...
    try:
        ...
        if agents_template.exists() and not agents_dest.exists():
            ...
            agents_dest.write_text("""# Amp Context for Pi

You are working in an Amp vault - a personal knowledge management system.
...
## Important
- Use the amp-mcp-bridge extension for task and calendar operations
- Preserve existing file formats and conventions
""")

        # Copy MCP bridge extension if template exists
        bridge_template = BETA_TEMPLATES_DIR / 'pi' / 'amp-mcp-bridge.ts'
        bridge_dest = pi_dir / 'extensions' / 'amp-mcp-bridge.ts'
...
def locked_yaml_update(filepath: Path, modifier_fn):
    """Load YAML, apply modifier, save with file locking."""
...
def validate_code(code: str) -> tuple[Optional[str], Optional[Dict]]:
    ...
    for feature_key, feature in features.items():
        ...
        if secrets.compare_digest(hash_code(salt, code), expected_hash):
...
def get_feature_instructions(feature_key: str) -> Optional[str]:
    ...
    full_path = (BASE_DIR / instructions_path).resolve()
...
async def _handle_call_tool_inner(
    ...
    if name == "validate_beta_code":
        ...
        # Load user profile (read-only check first)
        profile = load_user_profile()
        if not profile:
            result = create_error_response(
        ...
        # Check if already activated (read-only, no lock needed)
        if isinstance(profile.get('beta'), dict) and feature_key in profile['beta'].get('activated', {}):
            result = create_success_response({
                "feature_key": feature_key,
                "feature_name": feature.get('name'),
        ...
        # Run feature-specific setup before locked write
        if feature_key == 'pi':
            if not setup_pi_integration():
                logger.warning("Pi integration setup had issues, but activation continues")

        # Activate the feature with file locking to prevent concurrent overwrites
        _feature_key = feature_key
        _feature = feature

        def _activate_beta(data, _feature_key=_feature_key, _feature=_feature):
            if 'beta' not in data:
                data['beta'] = {
                    'beta_user': True,
                    'activated': {},
            ...
            data['beta']['beta_user'] = True
            if 'activated' not in data['beta']:
                data['beta']['activated'] = {}
            data['beta']['activated'][_feature_key] = {
                'activated_at': datetime.now().isoformat(),
                'version_at_activation': _feature.get('version'),
                'capabilities': _feature.get('capabilities', [])
            }
            return data

        try:
            updated_profile = locked_yaml_update(USER_PROFILE_FILE, _activate_beta)
        except Exception as e:
        ...
        result = create_success_response({
            ...
            "activated_at": updated_profile['beta']['activated'][feature_key]['activated_at']
...
async def _main():
    ...
    logger.info("Starting Amp Beta Features MCP Server")

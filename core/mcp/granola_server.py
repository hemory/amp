#!/usr/bin/env python3
"""
Granola Meeting Notes MCP Server for Amp
...
def _extract_attendee_info(attendee: dict) -> Optional[dict]:
    """Extract standardized attendee info from raw Granola data."""
    name = (
...
def _is_within_date_cutoff(created_at: str, cutoff_date: datetime) -> bool:
    """Check if a meeting date string is within the cutoff. Returns True if within range or unparseable."""
    if not created_at:
        return True
    try:
        meeting_date = datetime.fromisoformat(created_at.replace('Z', '+00:00'))
        return meeting_date.replace(tzinfo=None) >= cutoff_date
    except (ValueError, TypeError):
        logger.debug("Failed to parse meeting date: %s", created_at)
        return True
...
def convert_api_doc_to_meeting_info(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if doc.get('people', {}).get('attendees'):
        for attendee in doc['people']['attendees']:
            info = _extract_attendee_info(attendee)
            if info:
...
def read_granola_cache() -> Optional[Dict[str, Any]]:
    ...
    try:
        file_size = GRANOLA_CACHE.stat().st_size
        if file_size > 100_000_000:
            logger.warning("Granola cache too large (%d bytes), skipping", file_size)
            return None
...
def get_meetings_from_cache(
    ...
    for meeting_id, doc in cache['documents'].items():
        ...
        # Check date cutoff
        if not _is_within_date_cutoff(doc.get('created_at', ''), cutoff_date):
            continue
...
def search_meetings_in_cache(
    ...
    for meeting_id, doc in cache['documents'].items():
        ...
        for attendee in attendees:
            info = _extract_attendee_info(attendee)
            if info and query_lower in info['name'].lower():
                results.append(extract_meeting_info_from_cache(doc, cache['transcripts'], meeting_id))
                break
    ...
    if name == "granola_check_available":
        ...
        if not meetings:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("No meetings found. Both API and cache unavailable."), indent=2))]
        ...
        if not meeting_id:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("meeting_id is required"), indent=2))]
        
        meeting = get_meeting_details(meeting_id)
        
        if not meeting:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error(f"Meeting not found: {meeting_id}"), indent=2))]
        ...
        if not query:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("query is required"), indent=2))]
        ...
        if not dates:
            return [types.TextContent(type="text", text=json.dumps({
                **mcp_error("No dates found in meetings"),
                "has_data": False,
                "meetings_count": 0
            }, indent=2))]
...
async def _main():
    ...
    logger.info("Starting Amp Granola MCP Server (API-first with cache fallback)")
    logger.info(f"Granola credentials {'found' if GRANOLA_CREDS and Path(GRANOLA_CREDS).exists() else 'not found'}")

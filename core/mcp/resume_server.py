#!/usr/bin/env python3
"""
Resume Builder MCP Server for Amp
...
from core.mcp.resume_parser import (
    ResumeSession,
    Role,
    Achievement,
    Metric,
...
def load_session_from_disk(session_id: str) -> Optional[ResumeSession]:
    ...
    try:
        validate_vault_path(session_file, BASE_DIR)
    except ValueError:
        logger.warning(f"Path traversal blocked for session_id: {session_id}")
        return None
...
async def handle_load_session(arguments: dict) -> list[types.TextContent]:
    ...
    if not session:
        return [types.TextContent(
            type="text",
            text=json.dumps(mcp_error(f"Session not found: {session_id}"), indent=2)
        )]
...
async def handle_add_role(arguments: dict) -> list[types.TextContent]:
    ...
    if not validate_date_format(start_date):
        return [types.TextContent(
            type="text",
            text=json.dumps(mcp_error(f"Invalid start_date format: {start_date}. Use YYYY-MM"), indent=2)
        )]
    
    if not validate_date_format(end_date):
        return [types.TextContent(
            type="text",
            text=json.dumps(mcp_error(f"Invalid end_date format: {end_date}. Use YYYY-MM or 'present'"), indent=2)
        )]
...
async def handle_extract_achievements(arguments: dict) -> list[types.TextContent]:
    ...
    if not role:
        return [types.TextContent(
            type="text",
            text=json.dumps(mcp_error(f"Role not found: {role_id}"), indent=2)
        )]
...
async def handle_pull_career_evidence(arguments: dict) -> list[types.TextContent]:
    ...
    if not EVIDENCE_DIR.exists():
        return [types.TextContent(
            type="text",
            text=json.dumps(mcp_error("Career Evidence directory not found. Run /career-setup to initialize career system"), indent=2)
        )]
...
async def handle_generate_role_writeup(arguments: dict) -> list[types.TextContent]:
    ...
    if not role.achievements:
        return [types.TextContent(
            type="text",
            text=json.dumps(mcp_error("No achievements found for this role. Call extract_achievements first."), indent=2)
        )]
...
async def handle_generate_linkedin(arguments: dict) -> list[types.TextContent]:
    ...
    if not session.roles:
        return [types.TextContent(
            type="text",
            text=json.dumps(mcp_error("No roles found. Add roles first."), indent=2)
        )]
...
async def handle_export_resume(arguments: dict) -> list[types.TextContent]:
    ...
    # Validate path to prevent traversal via custom filename
    try:
        validate_vault_path(filepath, BASE_DIR)
    ...
    try:
        ...
        result = mcp_error(f"Failed to export resume: {str(e)}")
...
async def _main():
    ...
    logger.info("Starting Amp Resume Builder MCP Server")

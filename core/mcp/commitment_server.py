...
def locked_queue_operation(queue_path: Path, operation_fn):
    """Perform a queue operation with file locking."""
    ...
    with open(queue_path, 'r+') as f:
        ...
        try:
            ...
            f.seek(0)
            f.truncate()
            json.dump(result if result is not None else data, f, indent=2)
        finally:
...
async def _handle_call_tool_inner(name: str, arguments: dict):
    if name == "scan_for_commitments":
        ...
        if not is_beta_activated():
            return [TextContent(
                ...
                text=json.dumps({
                    ...
                    "message": "ScreenPipe is a beta feature. Run /beta-activate AMPSCREENPIPE2026 to enable.",
        ...
        _new_commitments = new_commitments
        _added_count = [0]

        def _add_commitments(queue, _new_commitments=_new_commitments, _added_count=_added_count):
            for comm in _new_commitments:
                is_dupe = any(
        ...
        locked_queue_operation(QUEUE_FILE, _add_commitments)
        added = _added_count[0]
        
        # Return summary (read-only, no lock needed)
        queue = load_queue()
        ...
        _commitment_id = commitment_id
        _action = action
        _found = [False]

        def _process(queue, _commitment_id=_commitment_id, _action=_action, _found=_found):
            for c in queue["commitments"]:
                if c["id"] == _commitment_id:
                    c["status"] = "processed"
                    c["processed_at"] = datetime.now().isoformat()
                    c["action_taken"] = _action
                    _found[0] = True
                    break
            if _found[0]:
                ...
                elif _action == "already_handled":
                    queue["stats"]["already_handled"] += 1
            return queue

        locked_queue_operation(QUEUE_FILE, _process)

#!/usr/bin/env python3
"""
MCP Server for Amp Session Memory System
Exposes conversation intelligence from the amp-app SQLite DB to Cursor/CLI.
...
DB location: System/.amp-sessions.db (shared with amp-app via WAL mode)
"""
...
DB_PATH = BASE_DIR / 'System' / '.amp-sessions.db'
...
def get_db() -> sqlite3.Connection:
    ...
    if not DB_PATH.exists():
        raise FileNotFoundError(f"Sessions DB not found at {DB_PATH}. Start the Amp app first to create it.")
...
async def handle_list_tools() -> list[types.Tool]:
    return [
        types.Tool(
            ...
            description=(
                "Search across Amp app conversation sessions and messages using full-text search. "
        ...
        types.Tool(
            ...
            description=(
                "Search observations (tool calls, decisions, insights) logged during Amp app sessions. "
                "Observations capture what Amp actually did: files written, tasks created, decisions made."
            ),
            inputSchema={
                "type": "object",
                "properties": {
        ...
        types.Tool(
            ...
            description=(
                ...
                "Shows what Amp has done involving this person, company, or project."
            ),
            inputSchema={
                "type": "object",
                "properties": {
        ...
        types.Tool(
            ...
            description=(
                "See what tools Amp has used recently in the app. "
...
async def handle_call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
    ...
    try:
        ...
        return [types.TextContent(type="text", text=json.dumps({
            "error": str(e),
            "hint": "The Amp app creates this database. Start the app and have at least one conversation first."
        }))]
...
async def _main():
    logger.info("Starting Amp Session Memory MCP Server")
    ...
    async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
        await app.run(
            ...
            InitializationOptions(
                server_name="amp-session-memory",

#!/usr/bin/env python3
"""
MCP Server for Amp Work Management System
...
from core.constants import (
    ...
    TASKS_FILE as TASKS_FILE_NAME,
    QUARTER_GOALS_FILE as QUARTER_GOALS_FILE_NAME,
    WEEK_PRIORITIES_FILE as WEEK_PRIORITIES_FILE_NAME,
    PEOPLE_DIR as PEOPLE_DIR_NAME,
...
def locked_read_modify_write(filepath: Path, modifier_fn):
    """Read a file, apply modifier_fn to its content, write back with file locking."""
...
if not _vault_path:
    logging.warning("VAULT_PATH not set. Set VAULT_PATH in your environment or run the install script. Falling back to cwd().")
BASE_DIR = Path(_vault_path) if _vault_path else Path.cwd()
TASKS_FILE = BASE_DIR / TASKS_FILE_NAME
WEEK_PRIORITIES_FILE = BASE_DIR / WEEK_PRIORITIES_DIR_NAME / 'Week_Priorities.md'
QUARTER_GOALS_FILE = BASE_DIR / QUARTER_GOALS_FILE_NAME
GOALS_FILE = BASE_DIR / 'GOALS.md'  # Legacy, kept for compatibility
INBOX_DIR = BASE_DIR / INBOX_DIR_NAME
PILLARS_FILE = BASE_DIR / SYSTEM_DIR_NAME / 'pillars.yaml'
SKILL_RATINGS_FILE = BASE_DIR / SYSTEM_DIR_NAME / 'Skill_Ratings' / 'ratings.jsonl'
COMPANIES_DIR = BASE_DIR / COMPANIES_DIR_NAME
PEOPLE_DIR = BASE_DIR / PEOPLE_DIR_NAME
MEETINGS_DIR = BASE_DIR / INBOX_DIR_NAME / 'Meetings'
PEOPLE_INAMP_FILE = BASE_DIR / SYSTEM_DIR_NAME / 'People_Index.json'
MEETING_CACHE_FILE = BASE_DIR / SYSTEM_DIR_NAME / 'Memory' / 'meeting-cache.json'

USER_PROFILE_FILE = BASE_DIR / SYSTEM_DIR_NAME / 'user-profile.yaml'


def validate_vault_path(user_path: str, base_dir: Path = None) -> Path:
    """Validate that a user-supplied path stays within the vault boundary."""
    if base_dir is None:
        base_dir = BASE_DIR
    resolved = (base_dir / user_path).resolve()
    if not resolved.is_relative_to(base_dir.resolve()):
        raise ValueError(f"That path goes outside your vault. Use a path inside your vault folder (got: '{user_path}').")
    return resolved


def get_tasks_file() -> Path:
    """Get the 03-Tasks/Tasks.md file."""
    return TASKS_FILE

def get_pillars_file() -> Path:
    """Get the pillars.yaml file."""
    return PILLARS_FILE

def get_week_priorities_file() -> Path:
    """Get the Week Priorities file."""
...
TASK_ID_NUM_RE = re.compile(r'\^task-\d{8}-(\d{3})')
TASK_TITLE_RE = re.compile(r'-\s*\[[x ]\]\s*\*?\*?(.+?)\*?\*?\s*\^')
TASK_TITLE_LINE_RE = re.compile(r'-\s*\[[x ]\]\s*\*?\*?(.+?)\*?\*?(?:\s*\|.*)?$')
TASK_TITLE_LINKED_RE = re.compile(r'-\s*\[[x ]\]\s*\*?\*?(.+?)\*?\*?(?:\s*\^task-|\s*\|)')
TASK_TITLE_FULL_RE = re.compile(r'-\s*\[[x ]\]\s*\*?\*?(.+?)\*?\*?(?:\s*\^task-\d{8}-\d{3})?\s*$')
TASK_OPEN_TITLE_RE = re.compile(r'-\s*\[ \]\s*\*?\*?(.+?)\*?\*?(?:\s*\^|\s*$)')
COMPLETION_TIMESTAMP_RE = re.compile(r'\s*\u2705\s*\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}')

# File reference patterns
FILE_PATH_REF_RE = re.compile(r'(?:People|Active)/[A-Za-z0-9_/-]+(?:\.md)?')
MD_FILE_REF_RE = re.compile(r'(?<!\[)\b([A-Za-z0-9_/-]+\.md)\b(?!\])')
PIPE_PATH_REF_RE = re.compile(r'\s*\|\s*(?:People|Active)/[^\s]+')
MD_EXT_SPACE_RE = re.compile(r'\s+\.md\b')
PIPE_TRAILING_RE = re.compile(r'\s*\|.*$')
TASK_ID_INLINE_RE = re.compile(r'\s*\^task-\d{8}-\d{3}\s*')

# Priority pattern
PRIORITY_RE = re.compile(r'Priority:\s*(P[0-3])')

# Section patterns (with DOTALL)
RELATED_TASKS_SECTION_RE = re.compile(r'## Related Tasks\n.*?(?=\n## |\n# |\Z)', re.DOTALL)
KEY_CONTACTS_SECTION_RE = re.compile(r'## Key Contacts\n.*?(?=\n## |\Z)', re.DOTALL)
MEETING_HISTORY_SECTION_RE = re.compile(r'## Meeting History\n.*?(?=\n## |\Z)', re.DOTALL)
RELATED_TASKS_COMPANY_RE = re.compile(r'## Related Tasks\n.*?(?=\n## |\Z)', re.DOTALL)
UPDATED_TIMESTAMP_RE = re.compile(r'\*Updated: .*?\*')

# Meeting file parsing
FRONTMATTER_RE = re.compile(r'^---\n(.*?)\n---', re.DOTALL)
FRONTMATTER_KV_RE = re.compile(r'^(\w+):\s*(.+)')
TITLE_HEADING_RE = re.compile(r'^# (.+)$', re.MULTILINE)
MD_EXT_RE = re.compile(r'\.md$')
FILENAME_DATE_PREFIX_RE = re.compile(r'^\d{4}-\d{2}-\d{2}\s*-?\s*')
SECTION_HEADING_RE = re.compile(r'^## ', re.MULTILINE)

# Meeting section item cleanup
CHECKBOX_PREFIX_RE = re.compile(r'^\[[ x]\]\s*')
TASK_ID_SUFFIX_RE = re.compile(r'\s*\^task-\d{8}-\d{3}\s*$')
WIKILINK_ALIAS_RE = re.compile(r'\[\[[^\]|]*\|([^\]]*)\]\]')
WIKILINK_RE = re.compile(r'\[\[([^\]]*)\]\]')
BOLD_RE = re.compile(r'\*\*([^*]+)\*\*')

# Goal patterns
GOAL_ID_RE = re.compile(r'\^(Q\d+-\d{4}-goal-\d+)')
GOAL_HEADER_RE = re.compile(r'###\s+(\d+)\.\s+(.+?)\s+\u2014\s+\*\*(.+?)\*\*(?:\s+\^(Q\d+-\d{4}-goal-\d+))?')
MILESTONE_RE = re.compile(r'-\s*\[([x ])\]\s*(.+)')
PROGRESS_PCT_RE = re.compile(r'(\d+)%')
CAREER_GOAL_RE = re.compile(r'\*\*Career goal:\*\*\s*(.+)')
SKILLS_DEV_RE = re.compile(r'\*\*Skills developing:\*\*\s*(.+)')
IMPACT_LEVEL_RE = re.compile(r'\*\*Impact level:\*\*\s*(low|medium|high)')
LINKED_PRIORITY_TITLE_RE = re.compile(r'(?:\d+\.\s+)?(.+?)\s+\u2014')
BOLD_TITLE_RE = re.compile(r'\*\*(.+?)\*\*')

# Priority ID patterns
PRIORITY_ID_RE = re.compile(r'\^(week-\d{4}-W\d{2}-p\d+)')
PRIORITY_LINE_RE = re.compile(r'(\d+)\.\s+(.+?)\s+\u2014\s+\*\*(.+?)\*\*(?:\s+\^(week-\d{4}-W\d{2}-p\d+))?')
LINKED_GOAL_REF_RE = re.compile(r'\[(Q\d+-\d{4}-goal-\d+)\]')

# Migration patterns
GOAL_HEADER_NO_ID_RE = re.compile(r'(###\s+\d+\.\s+.+?\s+\u2014\s+\*\*.*?\*\*)(?!\s+\^)')
QUARTER_FRONTMATTER_RE = re.compile(r'quarter:\s+(.+)')
WEEK_OF_RE = re.compile(r'\*\*Week of:\*\*\s+(\d{4}-\d{2}-\d{2})')
PRIORITY_NO_ID_RE = re.compile(r'(\d+\.\s+.+?\s+\u2014\s+\*\*.*?\*\*)(?!\s+\^)')

# Project status
PROJECT_STATUS_RE = re.compile(r'status:\s*(.+?)(?:\n|$)')

# Commitment patterns
OPEN_ITEMS_SECTION_RE = re.compile(r'(?:## Open Items|## Action Items|## Follow-?ups?)\n(.*?)(?:\n##|\Z)', re.DOTALL)
...
_vault_file_cache = None
_vault_cache_time = 0
_VAULT_CACHE_TTL = 5  # seconds

def _get_vault_files(base_dir: Path, pattern: str = '*.md') -> list:
    """Get vault files with 5-second cache to avoid repeated rglob scans."""
    global _vault_file_cache, _vault_cache_time
    now = time.time()
    if _vault_file_cache is not None and (now - _vault_cache_time) < _VAULT_CACHE_TTL:
        return _vault_file_cache
    _vault_file_cache = list(base_dir.rglob(pattern))
    ...
    try:
        ...
        for folder_name in task_folders:
            ...
            if not folder.exists():
                continue
            for md_file in _get_vault_files(folder):
                try:
                    content = md_file.read_text()
                    matches = TASK_ID_NUM_RE.findall(content)
                    existing_ids.extend([int(m) for m in matches])
                except (OSError, ValueError) as e:
                    logger.debug("Failed to read task IDs from %s: %s", md_file, e)
                    continue
        ...
        logging.warning(f"Vault scan failed for task ID generation: {e}")
        import uuid
        return f"task-{date_str}-{uuid.uuid4().hex[:6]}"
...
def find_task_by_id(task_id: str) -> List[Dict[str, Any]]:
    ...
    for md_file in _get_vault_files(BASE_DIR):
        try:
            content = md_file.read_text()
            lines = content.split('\n')
            ...
            for i, line in enumerate(lines):
                if f'^{task_id}' in line and ('- [ ]' in line or '- [x]' in line):
                    # Extract task title
                    title_match = TASK_TITLE_RE.match(line.strip())
...
def update_task_status_everywhere(task_id: str, completed: bool) -> Dict[str, Any]:
    ...
    if not instances:
        return {
            'success': False,
            'error': f"Couldn't find a task with ID '{task_id}'. Double-check the ID, or search by title instead."
        }
    ...
    for instance in instances:
        try:
            ...
            _task_id = task_id
            _completion_timestamp = completion_timestamp
            _line_idx = line_idx

            def _update_task_line(content, _completed=_completed, _task_id=_task_id,
                                  _completion_timestamp=_completion_timestamp, _line_idx=_line_idx):
                lines = content.split('\n')
                if _line_idx >= len(lines):
                    return content
                old_line = lines[_line_idx]
                if _completed:
                    new_line = old_line.replace('- [ ]', '- [x]')
                    new_line = COMPLETION_TIMESTAMP_RE.sub('', new_line)
                    task_id_match = re.search(r'\^' + re.escape(_task_id), new_line)
                    if task_id_match:
                        insert_pos = task_id_match.end()
                        new_line = new_line[:insert_pos] + f' ✅ {_completion_timestamp}' + new_line[insert_pos:]
                else:
                    new_line = old_line.replace('- [x]', '- [ ]')
                    new_line = COMPLETION_TIMESTAMP_RE.sub('', new_line)
                lines[_line_idx] = new_line
                return '\n'.join(lines)

            old_content = filepath.read_text() if filepath.exists() else ''
            new_content = _update_task_line(old_content)
            if new_content != old_content:
                locked_read_modify_write(filepath, _update_task_line)
...
def extract_file_refs_from_task(task_line: str) -> List[str]:
    ...
    refs.extend(FILE_PATH_REF_RE.findall(task_line))
    
    # Also match explicit markdown file references
    refs.extend(MD_FILE_REF_RE.findall(task_line))
    ...
    while i < len(lines):
        ...
        if line.strip().startswith('- [ ]') or line.strip().startswith('- [x]'):
            ...
            if task_mentions_page:
                # Extract title
                title_match = TASK_TITLE_LINE_RE.match(line.strip())
                title = title_match.group(1).strip() if title_match else line.strip()[6:]
                
                # Clean title of file references for display
                clean_title = PIPE_PATH_REF_RE.sub('', title)
                clean_title = MD_EXT_SPACE_RE.sub('', clean_title)
                clean_title = PIPE_TRAILING_RE.sub('', clean_title)
                ...
                while j < len(lines) and lines[j].strip().startswith('\t-'):
                    if 'Priority:' in lines[j]:
                        priority_match = PRIORITY_RE.search(lines[j])
                        if priority_match:
...
def update_related_tasks_section(page_path: str, tasks: List[Dict[str, Any]]) -> bool:
    ...
    filepath = validate_vault_path(page_path)
    if not page_path.endswith('.md'):
        filepath = validate_vault_path(f"{page_path}.md")
    ...
    if RELATED_TASKS_SECTION_RE.search(content):
        # Replace existing section
        new_content = RELATED_TASKS_SECTION_RE.sub(section_content.rstrip(), content)
    else:
...
def build_people_inamp_data() -> Dict[str, Any]:
    ...
    # Write to file
    PEOPLE_INAMP_FILE.parent.mkdir(parents=True, exist_ok=True)
    atomic_write(PEOPLE_INAMP_FILE, json.dumps(index, indent=2, cls=DateTimeEncoder) + '\n')
...
def lookup_person_data(name: str, company: str = None) -> Dict[str, Any]:
    ...
    index = None
    if PEOPLE_INAMP_FILE.exists():
        try:
            index = json.loads(PEOPLE_INAMP_FILE.read_text())
        except (json.JSONDecodeError, OSError):
            pass

    # Auto-rebuild if index is missing or stale (>24 hours old)
    if not index:
        index = build_people_inamp_data()
    else:
...
def rebuild_meeting_cache_data() -> Dict[str, Any]:
    ...
    if not meetings_dir.exists():
        return {'success': False, 'error': 'No meetings directory found. Meeting notes go in 00-Inbox/Meetings/. Process some meetings first with /process-meetings.'}

    files = [f for f in meetings_dir.glob('*.md') if f.name != 'README.md']
    if not files:
        return {'success': False, 'error': 'No meeting files found in your meetings directory. Add meeting notes or run /process-meetings to sync from Granola.'}
    ...
    for filepath in files:
        ...
        date_match = DATE_RE.search(filepath.name)
        ...
        try:
            ...
            logger.debug("Skipped meeting file processing: %s", e)
            skipped += 1
    ...
    atomic_write(MEETING_CACHE_FILE, json.dumps(cache, indent=2) + '\n')
...
def _parse_meeting_file_python(content: str, filename: str, rel_path: str) -> Dict[str, Any]:
    ...
    # Title
    title_match = TITLE_HEADING_RE.search(content)
    title = title_match.group(1).strip() if title_match else (
        MD_EXT_RE.sub('', FILENAME_DATE_PREFIX_RE.sub('', filename)).strip()
    )
    ...
    def extract_section(heading):
        ...
        if not match:
            return []
        start = match.end()
        end_match = SECTION_HEADING_RE.search(content[start:])
        ...
        for line in block.split('\n'):
            ...
            if stripped.startswith('- '):
                ...
                item = CHECKBOX_PREFIX_RE.sub('', item)
                item = TASK_ID_SUFFIX_RE.sub('', item)
                item = WIKILINK_ALIAS_RE.sub(r'\1', item)
                item = WIKILINK_RE.sub(r'\1', item)
                item = BOLD_RE.sub(r'\1', item)
                if item:
                    items.append(item)
        return items
...
def refresh_company_page(company_path: str) -> Dict[str, Any]:
    ...
    if company_path.startswith('Active/'):
        filepath = validate_vault_path(company_path)
    else:
        filepath = validate_vault_path(Path(company_path).name, base_dir=COMPANIES_DIR)
    
    if not filepath.exists():
        return {
            'success': False,
            'error': f"Couldn't find a company page at {filepath}. Check the company name or create one first."
        }
    ...
    # Key Contacts
    if KEY_CONTACTS_SECTION_RE.search(content):
        content = KEY_CONTACTS_SECTION_RE.sub(contacts_section, content)
    
    # Meeting History
    if MEETING_HISTORY_SECTION_RE.search(content):
        content = MEETING_HISTORY_SECTION_RE.sub(meetings_section, content)
    
    # Related Tasks
    if RELATED_TASKS_COMPANY_RE.search(content):
        content = RELATED_TASKS_COMPANY_RE.sub(tasks_section, content)
    
    # Update the Updated timestamp at the bottom
    content = UPDATED_TIMESTAMP_RE.sub(f'*Updated: {timestamp}*', content)
...
def create_company_page(name: str, website: str = '', industry: str = '', 
    ...
    if filepath.exists():
        return {
            'success': False,
            'error': f'A company page already exists at {filepath}. Open the existing page or use a different name.'
        }
...
def parse_quarterly_goals(filepath: Path) -> List[Dict[str, Any]]:
    ...
    while i < len(lines):
        ...
        goal_match = GOAL_HEADER_RE.match(line)
        if goal_match:
            ...
            while j < len(lines) and not lines[j].startswith('###'):
                if '**What success looks like:**' in lines[j]:
                    ...
                    milestone_match = MILESTONE_RE.match(lines[j].strip())
                    if milestone_match:
                        milestones.append({
    ...
    for i, line in enumerate(lines):
        ...
        if goal_id in line:
            ...
            if '**' in line and ('- [ ]' in line or '- [x]' in line or line.strip().startswith('1.') or line.strip().startswith('2.') or line.strip().startswith('3.')):
                ...
                # Extract title
                title_match = LINKED_PRIORITY_TITLE_RE.search(line)
                if not title_match:
                    title_match = BOLD_TITLE_RE.search(line)
                title = title_match.group(1).strip() if title_match else line.strip()
...
"""
        atomic_write(goals_file, content)
...
def extract_priority_id(text: str) -> Optional[str]:
    ...
    match = PRIORITY_ID_RE.search(text)
    return match.group(1) if match else None
...
def parse_weekly_priorities(filepath: Path) -> List[Dict[str, Any]]:
    ...
    for i, line in enumerate(lines):
        ...
        priority_match = PRIORITY_LINE_RE.match(line)
        if priority_match:
            ...
            if j < len(lines) and 'Quarterly goal:' in lines[j]:
                goal_match = LINKED_GOAL_REF_RE.search(lines[j])
                if goal_match:
...
def find_linked_tasks(priority_id: str) -> List[Dict[str, Any]]:
    ...
    for i, line in enumerate(lines):
        ...
        if priority_id in line and ('- [ ]' in line or '- [x]' in line):
            ...
            # Extract title
            title_match = TASK_TITLE_LINKED_RE.match(line.strip())
            title = title_match.group(1).strip() if title_match else line.strip()
...
def parse_tasks_file(filepath: Path) -> List[Dict[str, Any]]:
    ...
    for i, line in enumerate(lines):
        ...
        if line.strip().startswith('- [ ]') or line.strip().startswith('- [x]'):
            ...
            title_match = TASK_TITLE_FULL_RE.match(line.strip())
            title = title_match.group(1).strip() if title_match else line.strip()[6:]
            
            # Clean title - remove file path references for display
            clean_title = PIPE_PATH_REF_RE.sub('', title)
            clean_title = MD_EXT_SPACE_RE.sub('', clean_title)
            clean_title = TASK_ID_INLINE_RE.sub('', clean_title)
...
def find_similar_tasks(item: str, existing_tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for task in existing_tasks:
        ...
        keyword_overlap = _jaccard_similarity(item_keywords, task_keywords)
...
def migrate_quarterly_goals() -> Dict[str, Any]:
    ...
    for i, line in enumerate(lines):
        ...
        goal_match = GOAL_HEADER_NO_ID_RE.match(line)
        if goal_match:
            # This goal doesn't have an ID, add one
            goal_header = goal_match.group(1)
            
            # Generate ID
            quarter_match = QUARTER_FRONTMATTER_RE.search(content[:content.find(line)])
...
def migrate_weekly_priorities() -> Dict[str, Any]:
    ...
    # Determine week date
    week_match = WEEK_OF_RE.search(content)
    if week_match:
    ...
    for i, line in enumerate(lines):
        ...
        priority_match = PRIORITY_NO_ID_RE.match(line)
        if priority_match:
        ...
        try:
            ...
            if score > best_score:
                ...
                if 'status:' in content_lower:
                    status_match = PROJECT_STATUS_RE.search(content_lower)
                    if status_match:
            ...
            logger.debug("Error scanning file: %s", e)
            continue
        ...
        for attendee in attendees:
            ...
            for line in content.split('\n'):
                if '- [ ]' in line and attendee_lower in line.lower():
                    # Extract task title
                    title_match = TASK_OPEN_TITLE_RE.match(line.strip())
                    if title_match:
    ...
    for subdir in ['External', 'Internal']:
        ...
        for person_file in people_subdir.glob('*.md'):
            try:
                ...
                open_items_match = OPEN_ITEMS_SECTION_RE.search(content)
                if open_items_match:
                    section_content = open_items_match.group(1)
                    # Extract uncompleted items
                    for line in section_content.split('\n'):
                        if '- [ ]' in line:
                            item_text = UNCHECKED_ITEM_RE.sub('', line).strip()
...
async def _handle_call_tool_inner(
    ...
    if name == "list_tasks":
        ...
        if pillar not in PILLARS:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error(f"'{pillar}' isn't a valid pillar. Choose one of: {list(PILLARS.keys())}"), indent=2))]
        
        # Validate priority
        if priority not in PRIORITIES:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error(f"'{priority}' isn't a valid priority level. Choose one of: {PRIORITIES}"), indent=2))]
        
        # Check ambiguity
        if is_ambiguous(title):
            questions = generate_clarification_questions(title)
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("That task title is too vague to be actionable. Try adding specifics: what exactly needs to happen, and for whom?"), indent=2))]
        ...
        if similar:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("This looks like a duplicate of an existing task. Want me to search for similar tasks?"), indent=2))]
        ...
        if priority in PRIORITY_LIMITS and priority_counts.get(priority, 0) >= PRIORITY_LIMITS[priority]:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error(f"You've hit the limit for {priority} tasks. Complete or deprioritize some existing {priority} items first, or use a lower priority."), indent=2))]
        ...
        # Add to 03-Tasks/Tasks.md under the appropriate section (locked)
        _section = section
        _task_entry = task_entry

        def _add_task_to_content(content, _section=_section, _task_entry=_task_entry):
            if not content.strip():
                content = "# Tasks\n\n"
            section_header = f"## {_section}"
        ...
        locked_read_modify_write(tasks_file, _add_task_to_content)
        ...
        # If task_title provided, find the task and get its ID
            ...
            if not matching:
                return [types.TextContent(type="text", text=json.dumps(
                    mcp_error(f"Couldn't find a task matching '{task_title}'. Check the title or try a broader search term."), indent=2))]
            ...
            # Legacy support: task without ID, update only in source file
                ...
                _line_idx = task['line_number'] - 1
                _new_status = new_status

                def _update_legacy_task(content, _line_idx=_line_idx, _new_status=_new_status):
                    lines = content.split('\n')
                    if _line_idx >= len(lines):
                        return content
                    old_line = lines[_line_idx]
                ...
                locked_read_modify_write(filepath, _update_legacy_task)
            ...
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("Need either a task_id or task_title to find the task. Provide one of those and try again."), indent=2))]
        ...
        if not items:
            return [types.TextContent(type="text", text=json.dumps({
                "error": "No items to process. Pass a list of inbox items to triage."
            }, indent=2))]
        ...
        if not goal:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error(f"Couldn't find goal '{goal_id}'. Check the goal ID or run list_quarterly_goals to see available goals."), indent=2))]
        ...
        if not (0 <= progress_pct <= 100):
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("Progress must be a number between 0 and 100."), indent=2))]
        ...
        if not success:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error(f"Couldn't find goal '{goal_id}'. It may have been removed or the ID is wrong."), indent=2))]
        ...
        atomic_write(priorities_file, new_content)
        ...
        if not priorities_file.exists():
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error("Week Priorities file not found. Run /week-plan to create one, or check that 02-Week_Priorities/Week_Priorities.md exists."), indent=2))]
        ...
        if not priority:
            return [types.TextContent(type="text", text=json.dumps(
                mcp_error(f"Couldn't find priority '{priority_id}'. Check the ID or run get_week_progress to see current priorities."), indent=2))]
    ...
    # ========== NEW PLANNING INTELLIGENCE HANDLERS ==========
        ...
        result = build_people_inamp_data()
        return [types.TextContent(type="text", text=json.dumps({
            'success': True,
            'total': result['total'],
            'by_type': result['by_type'],
            'inamp_path': str(PEOPLE_INAMP_FILE),
        ...
        if not skill_name:
            return [types.TextContent(type="text", text=json.dumps(mcp_error("skill_name is required. Pass the name of the skill to rate (e.g., 'daily-plan').")))]
        if not (1 <= rating <= 5):
            return [types.TextContent(type="text", text=json.dumps(mcp_error("Rating must be a number from 1 to 5.")))]
...
async def _main():
    ...
    logger.info(f"Starting Amp Work MCP Server")

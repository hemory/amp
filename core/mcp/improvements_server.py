#!/usr/bin/env python3
"""
MCP Server for Amp Improvements Backlog System

Provides tools for capturing and managing Amp system improvement ideas with:
...
from core.utils import validate_input, mcp_error
...
BACKLOG_FILE = BASE_DIR / 'System' / 'Amp_Backlog.md'
...
# Keywords that indicate a changelog entry is relevant to Amp
AMP_RELEVANCE_KEYWORDS = [
    ...
    if HAS_QMD and is_qmd_available():
        try:
            results = vault_search(
                ...
                limit=5,
                min_score=0.3,
                fallback_glob="System/Amp_Backlog.md"
            )
            for r in results:
...
def parse_backlog_file() -> List[Dict[str, Any]]:
    """Parse the Amp backlog file and extract all ideas"""
    if not BACKLOG_FILE.exists():
        return []
...
def score_changelog_relevance(feature_text: str) -> int:
    """Score how relevant a changelog entry is to Amp (0-100)"""
    text_lower = feature_text.lower()
    score = 0

    for keyword in AMP_RELEVANCE_KEYWORDS:
...
def enrich_idea_in_backlog(idea_id: str, evidence: str, source: str) -> Dict[str, Any]:
    ...
    if not BACKLOG_FILE.exists():
        return mcp_error("Backlog file not found")
    ...
    if not match:
        return mcp_error(f"Idea {idea_id} not found in backlog")
...
def insert_idea_into_priority_queue(idea_id: str, title: str, description: str, category: str, score: int = 0, author: str = None, source: str = None) -> bool:
    ...
    lines.append(f"  - **Score:** {score}{'' if score > 0 else ' (not yet ranked - run `/amp-backlog` to calculate)'}")
...
def _scan_skill_names() -> List[Dict[str, str]]:
    ...
    for skill_dir in skills_dir.iterdir():
        ...
        try:
            ...
            logger.debug("Failed to read skill file %s: %s", skill_dir.name, e)
...
def _scan_mcp_tool_names() -> List[Dict[str, str]]:
    ...
    for py_file in mcp_dir.glob('*.py'):
        ...
        try:
            ...
            logger.debug("Failed to read MCP file %s: %s", py_file.name, e)
            continue
    return tools
...
def _scan_shipped_wip() -> List[Dict[str, str]]:
    ...
    try:
        ...
        logger.debug("Failed to scan file: %s", e)
        pass
    return shipped
...
def initialize_backlog_file():
    """Create the Amp backlog file with initial structure"""
    SYSTEM_DIR.mkdir(parents=True, exist_ok=True)
    
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M')
    
    content = f"""# Amp System Improvement Backlog

*Last ranked: {timestamp}*

Welcome to your Amp system improvement backlog! This file tracks ideas for making Amp better.

## How It Works

1. **Capture ideas** anytime using the `capture_idea` MCP tool
2. **Review regularly** with `/amp-backlog` to see AI-ranked priorities
3. **Workshop ideas** by running `/amp-improve [idea title]`
...
<!-- Auto-ranked by /amp-backlog command -->
...
*Run `/amp-backlog` to rank your ideas based on current system state.*
"""
    
    BACKLOG_FILE.write_text(content)
    logger.info(f"Created Amp backlog file at {BACKLOG_FILE}")
...
def mark_idea_implemented(idea_id: str, implementation_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not BACKLOG_FILE.exists():
        return {
            'success': False,
            'error': 'Amp backlog file does not exist'
        }
...
app = Server("amp-improvements-mcp")
...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        types.Tool(
            name="capture_idea",
            description="Capture a new Amp system improvement idea. Always available for quick capture from any context.",
            inputSchema={
                "type": "object",
                "properties": {
                    "title": {
        ...
        types.Tool(
            ...
            description="Scan Anthropic Claude Code changelog for new features relevant to Amp. Creates new backlog ideas or enriches existing ones with 'Why Now?' evidence. Call during /amp-whats-new or /daily-plan.",
            inputSchema={
                "type": "object",
                "properties": {
                    "days_back": {
        ...
        types.Tool(
            ...
            description="Run redundancy and staleness checks on all active backlog ideas. Checks against existing skills, MCP tools, shipped WIP items, and capabilities reports. Returns flagged ideas to kill, downrank, or archive. Call before /amp-backlog scoring or after /capabilities-report.",
            inputSchema={
                "type": "object",
                "properties": {}
            }
...
async def _handle_call_tool_inner(
    ...
    if name == "capture_idea":
        try:
            title = validate_input(arguments['title'], 'title')
            description = validate_input(arguments['description'], 'description', max_length=2000)
        except ValueError as e:
            return [types.TextContent(
                type="text",
                text=json.dumps(mcp_error(str(e)), indent=2)
            )]
        ...
        if category not in CATEGORIES:
            return [types.TextContent(
                type="text",
                text=json.dumps(
                    mcp_error(f"Invalid category '{category}'. Must be one of: {CATEGORIES}"), indent=2)
            )]
        ...
        if similar and similar[0]['similarity'] > 0.75:
            return [types.TextContent(
                type="text",
                text=json.dumps({
                    **mcp_error("Potential duplicate detected"),
        ...
        # Generate ID and add to Amp backlog
        ...
        if success:
            result = {
                ...
                "title": title,
                "category": category,
                "message": f"Idea captured successfully! Run `/amp-backlog` to see it ranked against other ideas.",
                "next_steps": [
                    "Run `/amp-backlog` to see AI-powered ranking",
                    "Run `/amp-improve \"{title}\"` to workshop this idea",
                    "Check `System/Amp_Backlog.md` to see all your ideas"
                ]
            }
            try:
            ...
            result = mcp_error("Failed to add idea to Amp backlog")
        ...
        result = {
            ...
            "note": "Scores are calculated by `/amp-backlog`. Run it to get AI-powered rankings."
        }
        ...
        if not idea:
            result = mcp_error(f"Idea {idea_id} not found")
        else:
            result = {
                "success": True,
                "idea": idea
        ...
        result = {
            ...
            "note": "Run `/amp-backlog` to update scores based on current system state"
        }
        ...
        for entry in relevant_entries:
            ...
            if best_match and best_similarity > 0.4:
                ...
                description = (
                    ...
                    f"Evaluate how Amp could leverage this for user workflows."
                )
        ...
        result = {
            ...
            "message": f"Scanned {len(entries)} changelog entries. {len(relevant_entries)} were relevant to Amp. Created {ideas_created} new ideas, enriched {ideas_enriched} existing ideas."
        }
        return [types.TextContent(type="text", text=json.dumps(result, indent=2, cls=DateTimeEncoder))]
...
async def _main():
    ...
    if _HAS_HEALTH:
        _mark_healthy("amp-improvements-mcp")
    logger.info(f"Starting Amp Improvements MCP Server")
    ...
    async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
        await app.run(
            ...
            InitializationOptions(
                server_name="amp-improvements-mcp",

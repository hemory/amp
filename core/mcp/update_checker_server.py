#!/usr/bin/env python3
"""
Amp Update Checker MCP Server

Checks for Amp updates from GitHub and notifies users of new versions.
Writes a persistent .update-available file so Amp can remind users once
...
mcp = FastMCP("Amp Update Checker")
...
def get_last_check_time() -> datetime | None:
    ...
    try:
        ...
        logger.debug("Failed to read last check time: %s", e)
        return None
...
def read_update_notification() -> dict | None:
    ...
    try:
        ...
        logger.debug("Failed to read update notification: %s", e)
        return None


def clear_update_notification():
    """Delete .update-available file (called after successful /amp-update)."""
    path = _notification_path()
    if path.exists():
        path.unlink()
...
async def check_for_updates(force: bool = False) -> dict:
    """
    Check if a newer version of Amp is available on GitHub.
...
async def dismiss_update() -> dict:
    """
    Clear the update notification entirely (e.g. after successful /amp-update).
